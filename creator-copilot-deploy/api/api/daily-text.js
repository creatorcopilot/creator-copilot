...
exports.handler = async function(event, context) {
  try {
    ...
    const { data: clients, error } = await supabase
      .from('clients')
      .select('id, name, business_name, phone, email, tier, location, goal, targeting, category, created_at, delivery_count')
      .eq('status', 'active')
      .not('phone', 'is', null);
...
function formatScriptText(script, firstName, dayOfWeek, location, isLocal, category, businessName) {
  ...
  if (isWeekend) {
    return `Creator Copilot 🎬 [${businessName}] Saturday — Weekend Post
